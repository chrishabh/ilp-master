...
if (! function_exists('envparam')) {
    ...
    function getFinalSubmissionXlsxFile($details,$file,$date){
        //Give our xlsx file a name.
        $xlsxFileName = $file.'_'.$date.'.xlsx';
           ...
           foreach($details as $i => $row){
            ...
            foreach($row as $x => $x_value) {
              ...
              $doc->getActiveSheet()->getStyle('A2:' . $last_column . $last_row)
                ...
                foreach($highlight_row as $highlight_value){
                    $range = "A$highlight_value:" . $last_column . $highlight_value;
                    $doc->getActiveSheet()
                    ->getStyle($range)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
    ...
    function importConstructionExcel($filePath)
    {
        ini_set('memory_limit', '-1');
        ini_set('post_max_size', '-1');
        ini_set('upload_max_filesize', '-1');
        ...
        if(!empty($filePath)){
            ...
            foreach ($reader->getSheetIterator() as $sheet_key => $sheet) {
            
                if($sheet_key == 0 && $sheet_key == 1){
                    continue;
                }
                $key = $key1 = $key2 =0;
                $total_insert = [];
